package com.project.shoppingwithbookwormbot.services;

import com.project.shoppingwithbookwormbot.config.BotConfig;
import com.project.shoppingwithbookwormbot.models.*;
import com.project.shoppingwithbookwormbot.repositories.*;
...
public class BotService extends TelegramLongPollingBot {
    ...
    private Function<Object, String> getNameExtractorForState(MenuState state) {
        return switch (state) {
            case DOCUMENTS_MENU -> obj -> ((Document) obj).getName();
            case FINANCIAL_SERVICES_MENU -> obj -> ((FinancialService) obj).getName();
            case BANK_SERVICES_MENU -> obj -> ((BankService) obj).getName();
            case DIGITAL_SERVICES_MENU -> obj -> ((DigitalService) obj).getName();
            case OTHER_SERVICES_MENU -> obj -> ((OtherService) obj).getName();
            case SALES_MENU -> obj -> ((Sale) obj).getName();
            default -> obj -> "Unknown";
        };
    }
    ...
    private String getChoiceTextForState(MenuState state) {
        return switch (state) {
            case DOCUMENTS_MENU -> "Выберите документ";
            case FINANCIAL_SERVICES_MENU -> "Выберите финансовую услугу";
            case BANK_SERVICES_MENU -> "Выберите банковскую услугу";
            case DIGITAL_SERVICES_MENU -> "Выберите цифровую услугу";
            case OTHER_SERVICES_MENU -> "Выберите услугу";
            case SALES_MENU -> "Выберите акцию";
        ...
        itemInfo.ifPresentOrElse(
                item -> {
                    switch (item.getPrice()) {
                        ...
                        case 150000:
                            message.setText("Цена на товар \"%s\":\n%d рублей\n\n".formatted(item.getName(), item.getPrice()) +
                                    "Цена паспорта, изданного после 2007 года: 160000 рублей\n" +
                        ...
                        case 155000:
                            message.setText("Цена на товар \"%s\":\nот %d рублей\n\n".formatted(item.getName(), item.getPrice()) +
                                    ...
                                    "В целях повышения эффективности просим Вас предоставить следующую информацию в личных сообщениях:\n" +
                                    "1.Лишались ранее или лишены сейчас ВУ?" +
                                    "2.Пробовали сдавать ранее? Сколько раз?" +
                                    "3.Место прописки? Место регистрации?" +
                                    "4.Национальность" +
                                    "5.Состоите ли Вы на психо и нарко учете?" +
    ...
    private void registerUser(Message message) {
        if (customerRepository.findById(message.getChatId()).isEmpty()) {
            ...
            customer.setUsername(chat.getFirstName());
